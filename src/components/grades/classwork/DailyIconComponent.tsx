...
export const DailyIconComponent = ({ icon, size = "h-4 w-4" }: { icon: DailyIcon; size?: string }) => {
  ...
  if (icon.level === "excellent") return <CircleCheck className={cn(size, "text-emerald-500")} />;
  if (icon.level === "average") return <CircleMinus className={cn(size, "text-amber-500")} />;
  return <CircleX className={cn(size, "text-rose-500")} />;
};
