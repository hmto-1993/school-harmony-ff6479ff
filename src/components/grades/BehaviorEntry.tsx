...
import BehaviorNoteDialog from "./behavior/BehaviorNoteDialog";
...
const DEFAULT_SUGGESTIONS: Record<string, string[]> = {
  positive: [
    "أظهر احتراماً للقوانين الصفية", "ساعد في ترتيب الفصل", "أظهر تعاوناً مع زملائه",
    "حافظ على الهدوء أثناء الدرس", "أظهر مسؤولية في أداء مهامه", "ساعد في حل النزاعات بين الطلاب",
    "أظهر قيادة إيجابية في المجموعة", "حافظ على النظام في الفصل",
  ],
  negative: [
    "أحدث ضوضاء في الفصل", "لم يتبع قوانين الفصل", "أزعج زملاءه أثناء الدرس",
    "لم يحترم دور المعلم", "أظهر سلوكاً غير لائق", "لم يشارك في أنشطة الفصل",
    "أظهر عدم احترام للآخرين", "أحدث فوضى في الفصل",
  ],
  neutral: [
    "أظهر سلوكاً مقبولاً في الفصل", "يحتاج تذكيراً ببعض القوانين",
    "أظهر تحسناً في السلوك", "أنجز المهام المطلوبة منه",
  ],
};
...
export default function BehaviorEntry({ selectedClass, onClassChange }: BehaviorEntryProps) {
  ...
  useEffect(() => {
    ...
    supabase.from("site_settings").select("id, value").in("id", [
      "behavior_suggestions_positive", "behavior_suggestions_negative", "behavior_suggestions_neutral",
    ]).then(({ data }) => {
  ...
  const loadData = async () => {
    ...
    const { data: records } = await supabase
      ...
      .in("student_id", studentIds).eq("date", today).eq("class_id", selectedClass);
    ...
    setStudents(
      (studs || []).map((s) => {
        ...
        return {
          student_id: s.id, full_name: s.full_name, parent_phone: s.parent_phone,
          type: (rec?.type as BehaviorType) || null, note, severity,
          existingId: rec?.id || null, notified: rec?.notified || false,
        };
      })
    );
  };
  ...
  const autoSaveRecord = useCallback(async (student: StudentBehavior, newType: BehaviorType, newNote?: string, newSeverity?: string) => {
    ...
    const buildNote = (type: BehaviorType, n: string, sev: string) => {
      const base = n || "";
      if (type === "negative" && sev && sev !== "low") return `[severity:${sev}] ${base}`.trim();
      if (type === "negative" && sev === "low") return base ? `[severity:low] ${base}`.trim() : `[severity:low]`;
      return base || null;
    };
    ...
    if (newType === null && student.existingId) {
      ...
      if (!controller.signal.aborted) {
        setStudents(prev => prev.map(s => s.student_id === student.student_id ? { ...s, type: null, existingId: null, notified: false } : s));
      }
      return;
    }

    if (newType === null) {
      setStudents(prev => prev.map(s => s.student_id === student.student_id ? { ...s, type: null } : s));
      return;
    }
    ...
    if (student.existingId) {
      await supabase.from("behavior_records").update({ type: newType, note: builtNote }).eq("id", student.existingId);
      if (!controller.signal.aborted) {
        setStudents(prev => prev.map(s => s.student_id === student.student_id ? { ...s, type: newType, note, severity } : s));
      }
    } else {
      const { data } = await supabase.from("behavior_records").insert({
        student_id: student.student_id, class_id: selectedClass, date: today,
        type: newType, note: builtNote, recorded_by: user.id,
      }).select("id").single();
      if (!controller.signal.aborted) {
        setStudents(prev => prev.map(s => s.student_id === student.student_id ? { ...s, type: newType, note, severity, existingId: data?.id || null } : s));
      }
    }
  ...
  const cycleType = (studentId: string) => {
    ...
    setStudents(prev => prev.map(s => s.student_id !== studentId ? s : { ...s, type: next }));
  ...
  const sendNotification = async (student: StudentBehavior) => {
    ...
    const { data, error } = await supabase.functions.invoke("send-sms", { body: { phone: student.parent_phone, message } });
    if (error || !data?.success) {
      ...
      if (student.existingId) await supabase.from("behavior_records").update({ notified: true }).eq("id", student.existingId);
      setStudents(prev => prev.map(s => s.student_id === student.student_id ? { ...s, notified: true } : s));
    }
  ...
  return (
    <>
                ...
                <table className="w-full text-sm border-separate border-spacing-0">
                  ...
                  <tbody>
                    {students.map((s, i) => {
                      ...
                      return (
                        <tr key={s.student_id} className={cn(isEven ? "bg-card" : "bg-muted/30 dark:bg-muted/20", !isLast && "border-b border-border/20", "hover:bg-sky-100/60 dark:hover:bg-sky-900/30 transition-colors")}>
                          ...
                          <td className="p-3 text-center border-l border-border/10">
                            <button type="button" onClick={() => cycleType(s.student_id)} className={cn("p-1.5 rounded-lg transition-all hover:scale-110 cursor-pointer mx-auto", s.type === "positive" && "bg-emerald-50 dark:bg-emerald-500/15", s.type === "neutral" && "bg-amber-50 dark:bg-amber-500/15", s.type === "negative" && "bg-rose-50 dark:bg-rose-500/15")} title="اضغط للتبديل">
                              <BehaviorIcon type={s.type} />
                            </button>
                          </td>
                          <td className="p-3 text-center border-l border-border/10">
                            <button type="button" onClick={() => setNoteDialog({ open: true, studentId: s.student_id, name: s.full_name })} className={cn("p-1 rounded-md transition-all hover:scale-110", s.note ? "text-primary" : "text-muted-foreground opacity-40 hover:opacity-70")} title={s.note || "إضافة ملاحظة"}>
                              <MessageSquare className="h-5 w-5" />
                            </button>
                          </td>
                          <td className={cn("p-3 text-center", isLast && "last:rounded-bl-xl")}>
                            <Button variant="ghost" size="sm" disabled={!s.type || !s.parent_phone || sendingNotif === s.student_id} onClick={() => sendNotification(s)} className={cn("h-8 px-2", s.notified && "text-green-600")} title={!s.parent_phone ? "لا يوجد رقم ولي أمر" : s.notified ? "تم الإرسال" : "إرسال إشعار"}>
                              {sendingNotif === s.student_id ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}
                            </Button>
                          </td>
                        </tr>
                      );
      ...
      <BehaviorNoteDialog
        ...
        student={currentNoteStudent}
        suggestions={suggestions}
        onTypeChange={(type) => {
          if (!currentNoteStudent) return;
          const newNote = currentNoteStudent.type !== type ? "" : currentNoteStudent.note;
          const newSeverity = type === "negative" ? currentNoteStudent.severity : "low";
          setStudents(prev => prev.map(s => s.student_id === noteDialog.studentId ? { ...s, type, note: newNote, severity: newSeverity } : s));
          autoSaveRecord(currentNoteStudent, type, newNote, newSeverity);
        }}
        onSeverityChange={(severity) => {
          setStudents(prev => prev.map(s => s.student_id === noteDialog.studentId ? { ...s, severity } : s));
        }}
        onNoteChange={(note) => {
          setStudents(prev => prev.map(s => s.student_id === noteDialog.studentId ? { ...s, note } : s));
        }}
      />
    </>
  );
