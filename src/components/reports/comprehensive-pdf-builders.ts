...
export async function buildComprehensivePDF(data: ComprehensiveData, summaryResult: string) {
  ...
  buildAttendanceDistributionPage(doc, data, tableStyles, pageWidth, pageHeight, chartX, chartW, barGap);
...
function buildPeriodProgressionPage(doc: any, data: ComprehensiveData, categories: any[], pageWidth: number, pageHeight: number) {
  doc.addPage("a4", "landscape");
  doc.setFontSize(14);
  doc.setFont("Amiri", "bold");
...
function buildTopBottomPage(doc: any, data: ComprehensiveData, allStudentAvgs: any[], categories: any[], tableStyles: any, pageWidth: number) {
  const circleR = 8;
...
function buildAttendanceDistributionPage(doc: any, data: ComprehensiveData, tableStyles: any, pageWidth: number, pageHeight: number, chartX: number, chartW: number, barGap: number) {
  const circleR = 8;
