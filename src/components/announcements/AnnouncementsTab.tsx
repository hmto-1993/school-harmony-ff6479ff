...
import { Megaphone, Send, Trash2, Globe, Users, Loader2, Pencil, Eye, EyeOff } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export default function AnnouncementsTab({ readOnly = false }: { readOnly?: boolean }) {
  ...
  const openEditDialog = (ann: Announcement) => {
    ...
    setTitle(ann.title);
    setBody(ann.body);
    setTargetType(ann.target_type as "all" | "classes");
    setSelectedClassIds(ann.target_class_ids || []);
    setDialogOpen(true);
  };
  ...
  const toggleActive = async (ann: Announcement) => {
    ...
    if (error) {
      ...
      toast({ title: "تم", description: newActive ? "تم تفعيل الإعلان" : "تم إيقاف الإعلان" });
      fetchAnnouncements();
    }
  };
    ...
    <div className="space-y-4">
      ...
      <Dialog open={dialogOpen} onOpenChange={(open) => { setDialogOpen(open); if (!open) resetForm(); }}>
        <DialogContent className="sm:max-w-lg" dir="rtl">
          <DialogHeader>
            <DialogTitle>{editingId ? "تعديل الإعلان" : "إرسال إعلان جديد"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSave} disabled={sending} className="gap-1.5">
              {sending ? <Loader2 className="h-4 w-4 animate-spin" /> : editingId ? <Pencil className="h-4 w-4" /> : <Send className="h-4 w-4" />}
              {sending ? "جارٍ الحفظ..." : editingId ? "حفظ التعديلات" : "إرسال الإعلان"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      {loading ? (
        ...
        announcements.map((ann) => (
          <Card key={ann.id} className={`shadow-card transition-opacity ${!ann.is_active ? "opacity-60" : ""}`}>
            <CardContent className="p-4">
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-start gap-3 flex-1">
                  <div className={`p-2 rounded-lg mt-0.5 ${ann.is_active ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground"}`}>
                    <Megaphone className="h-4 w-4" />
                  </div>
                ...
                <div className="flex items-center gap-1">
                  ...
                  <Button variant="ghost" size="icon" onClick={() => openEditDialog(ann)} className="text-muted-foreground hover:text-foreground">
                  ...
                  <AlertDialog>
                    ...
                    <AlertDialogContent dir="rtl">
                      <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>سيتم حذف الإعلان نهائياً. لا يمكن التراجع عن هذا الإجراء.</AlertDialogDescription>
                      </AlertDialogHeader>
                      <AlertDialogFooter>
                        <AlertDialogCancel>إلغاء</AlertDialogCancel>
                        <AlertDialogAction className="bg-destructive text-destructive-foreground hover:bg-destructive/90" onClick={() => handleDelete(ann.id)}>حذف</AlertDialogAction>
                      </AlertDialogFooter>
                    </AlertDialogContent>
                  </AlertDialog>
                </div>
