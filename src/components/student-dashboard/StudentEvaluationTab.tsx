...
export default function StudentEvaluationTab({ student, isParent, parentVis, evalSubView, setEvalSubView }: Props) {
  ...
  const LevelIcon = ({ level, isStar }: { level: string; isStar?: boolean }) => {
    if (isStar) return <Star className="h-5 w-5 text-amber-500 fill-amber-500 mx-auto" />;
    if (level === "star") return <Star className="h-5 w-5 text-amber-500 fill-amber-500 mx-auto" />;
    if (level === "excellent") return <CircleCheck className="h-5 w-5 text-emerald-600 mx-auto" />;
  ...
  const Legend = () => (
    <div className="flex items-center gap-4 mt-2 text-[10px] text-muted-foreground justify-center flex-wrap">
      ...
      <span className="flex items-center gap-1"><CircleCheck className="h-3.5 w-3.5 text-emerald-600" /> ممتاز</span>
