...
USING (
  id = ANY (ARRAY[
    ...
    'parent_show_daily_grades'::text, 'parent_show_classwork_icons'::text,
    'parent_classwork_icons_count'::text,
    'parent_show_library'::text, 'parent_show_activities'::text,
    'student_show_daily_grades'::text, 'student_show_classwork_icons'::text,
    'student_classwork_icons_count'::text
  ])
);
